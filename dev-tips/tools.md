...
7. [direnv](https://direnv.net/): loads and unloads environment variables automatically as you enter and leave a directory, driven by a per-folder `.envrc`. Handy for per-project secrets, tool versions, and (my main use) switching the `gh` CLI identity per repo. See [git-multiple-accounts.md](git-multiple-accounts.md).